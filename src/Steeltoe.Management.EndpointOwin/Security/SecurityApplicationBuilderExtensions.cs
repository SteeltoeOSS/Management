...
namespace Steeltoe.Management.EndpointOwin.Security
{
    public static class SecurityApplicationBuilderExtensions
    {
        ...
        public static void UseEndpointSecurity(this IAppBuilder builder, IConfiguration config, ILoggerFactory loggerFactory = null)
        {
            ...
            IManagementOptions cfmOptions = ManagementOptions.Get(config).OfType<ActuatorManagementOptions>().Single();
            var actuatorDiscoveryOptions = new ActuatorDiscoveryEndpointOptions(config);
            var logger = loggerFactory?.CreateLogger<ActuatorDiscoveryEndpointOwinMiddleware>();
            builder.Use<SecurityMiddleware>(actuatorDiscoveryOptions, cfmOptions, logger);
        }
    }
}
