...
{
    public class DiscoveryEndpointTest : BaseTest
    {
        [Fact]
        public void Constructor_ThrowsOptionsNull()
        {
            Assert.Throws<ArgumentNullException>(() => new ActuatorDiscoveryEndpoint(null, null));
        }
        ...
        public void Invoke_ReturnsExpectedLinks()
        {
            ...
            var cloudOpts = new ActuatorDiscoveryEndpointOptions();
            mgmtOptions.EndpointOptions.AddRange(new List<IEndpointOptions>() { infoOpts, cloudOpts });

            var ep = new ActuatorDiscoveryEndpoint(cloudOpts, new List<IManagementOptions>() { mgmtOptions });
        ...
        [Fact]
        public void Invoke_OnlyActuatorDiscoveryEndpoint_ReturnsExpectedLinks()
        {
            var cloudOpts = new ActuatorDiscoveryEndpointOptions();
            var mgmtOptions = new ActuatorManagementOptions();
        ...
        public void Invoke_CloudFoundryDisable_ReturnsExpectedLinks()
        {
            ...
            var cloudOpts = new ActuatorDiscoveryEndpointOptions { Enabled = false };
            var mgmtOptions = new ActuatorManagementOptions();
