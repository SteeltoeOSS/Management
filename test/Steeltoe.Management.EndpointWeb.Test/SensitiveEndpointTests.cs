...
namespace Steeltoe.Management.EndpointWeb.Test
{
    public class SensitiveEndpointTests
    {
        ...
        [Fact]
        public async void EnvEndpoint_ReturnsOKWhenSensitive()
        {
            var settings = _defaultSettings.Merge(new Settings
            {
                ["management:endpoints:env:sensitive"] = "true",
                ["management:endpoints:sensitiveclaim:type"] = "scope",
                ["management:endpoints:sensitiveclaim:value"] = "actuator.read",
            });
            using (var server = new TestServer(settings))
        ...
        [Fact]
        public async void EnvEndpoint_ReturnsUnauthorized()
        {
        ...
        [Fact]
        public async void LoggersEndpoint_ReturnsOKWhenSensitive()
        {
            var settings = _defaultSettings.Merge(new Settings
            {
            ...
            using (var server = new TestServer(settings))
            {
                ...
                var result = await client.GetAsync("http://localhost/management/loggers", "GET");

                Assert.Equal(HttpStatusCode.OK, result.StatusCode);
                Assert.NotNull(result);
                Assert.Contains("loggers", result.Content);
            }
        }

        [Fact]
        public async void LoggersEndpoint_ReturnsUnauthorized()
        {
            var settings = _defaultSettings.Merge(new Settings
            {
                ["management:endpoints:health:sensitive"] = "true"
        ...
        [Fact]
        public async void MetricsEndpoint_ReturnsOKWhenSensitive()
        {
            var settings = _defaultSettings.Merge(new Settings
            {
                ["management:endpoints:loggers:sensitive"] = "true",
            ...
            using (var server = new TestServer(settings))
            {
                ...
                var result = await client.GetAsync("http://localhost/management/metrics", "GET");

                Assert.Equal(HttpStatusCode.OK, result.StatusCode);
                Assert.NotNull(result);
                Assert.Contains("http.server", result.Content);
            }
        }

        [Fact]
        public async void MetricsEndpoint_ReturnsUnauthorized()
        {
            var settings = _defaultSettings.Merge(new Settings
            {
                ["management:endpoints:health:sensitive"] = "true"
        ...
        [Fact]
        public async void ThreadDumpEndpoint_ReturnsOKWhenSensitive()
        {
            var settings = _defaultSettings.Merge(new Settings
            {
                ["management:endpoints:loggers:sensitive"] = "true",
            ...
            using (var server = new TestServer(settings))
            {
                ...
                var result = await client.GetAsync("http://localhost/management/dump", "GET");

                Assert.Equal(HttpStatusCode.OK, result.StatusCode);
                Assert.NotNull(result);
                Assert.Contains("stackTrace", result.Content);
            }
        }

        [Fact]
        public async void ThreadDumpEndpoint_ReturnsUnauthorized()
        {
            var settings = _defaultSettings.Merge(new Settings
            {
                ["management:endpoints:health:sensitive"] = "true"
